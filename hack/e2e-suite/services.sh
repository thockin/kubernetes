...
function start_service() {
  ...
  svcs_to_clean=("${svcs_to_clean[@]:+${svcs_to_clean[@]}}" "$1")
}
...
#   $4: pod count
#   $5: pod IDs
# TODO: 'nc' on fedora has different args.  We can either switch to socat or
# run nc in a container.  Running in a container has some other issues on
# Vagrant (#1792), so defer for now. This test is broken on Vagrant+Fedora.
...
function wait_for_service_down() {
  for i in $(seq 1 20); do
    $(ssh-to-node "${test_node}" "
        nc $2 $3 -w 1 >/dev/null 2>&1 && exit 1 || exit 0
        ") && break
