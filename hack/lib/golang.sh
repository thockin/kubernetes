...
kube::golang::binaries_from_targets() {
  ...
  for target; do
    ...
    #FIXME: do we need this or can we make the fully-qualified case be default?
...
kube::golang::path_for_coverage_dummy_test() {
  local package="$1"
  local path=$(go list -find -f '{{.Dir}}' "${package}")
  local name
...
kube::golang::build_binaries() {
  ...
  (
    ...
    goflags+=("-modcacherw") # since we keep GOMODCACHE in _output
