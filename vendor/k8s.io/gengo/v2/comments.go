...
// marger argument should be unique enough to identify the tags needed, and
// should not be a marker for tags you don't want, or else the caller takes
// responsibility for making that distinction.
//
// The prefixes argument further selects among comment lines which match the
// marker.  If this is nil or empty, all lines which match the marker are
// considered.  If this is specified, only lines with begin with marker +
// prefix will be considered.  This is useful when a common marker is used which
// may match lines which fail this syntax (e.g. which predate this definition).
//
...
func ExtractFunctionStyleCommentTags(marker string, prefixes []string, lines []string) (map[string][]Tag, error) {
	...
	matches := func(line string) bool {
		...
		if len(prefixes) == 0 {
			//FIXME: should this fail, forcing callers to pass prefixes?  Not
			// passing them fails when it hits real tags like:
			// +protobuf.options.(gogoproto.goproto_stringer)=false
			return true
		}
		for _, pfx := range prefixes {
			if strings.HasPrefix(line, marker+pfx) {
				return true
			}
		}
		return false
	...
	for _, line := range lines {
		...
		tag := Tag{}
		if name, args, err := parseTagKey(key); err != nil {
			return nil, err
		} else {
...
//
// This function returns the key name and arguments.
func parseTagKey(input string) (string, []string, error) {
