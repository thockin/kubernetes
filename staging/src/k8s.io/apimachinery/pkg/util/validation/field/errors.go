...
// considered.
func IncompatibleWith(field *Path, other *Path) *Error {
	return IncompatibleWithValue(field, other, specified)
}
...
func IncompatibleWithValue(field *Path, other *Path, otherValue interface{}) *Error {
	rov := render(otherValue)
	detail := fmt.Sprintf("may not be specified when %s is %s", other.String(), rov)
	return &Error{ErrorTypeForbidden, field.String(), "", detail}
}
