...
func TestAtomicSlice(t *testing.T) {
	...
	st.Value(&AtomicSliceStruct{
		...
		AlphaEachValStandardValidation: []int{5},
		BetaEachValStandardValidation:  []int{5},
		...
		// Case: Alpha eachVal -> Alpha Error
		field.Invalid(field.NewPath("AlphaEachValStandardValidation").Index(0), 5, "").WithOrigin("minimum").MarkAlpha(),

		// Case: Beta eachVal -> Beta Error
		field.Invalid(field.NewPath("BetaEachValStandardValidation").Index(0), 5, "").WithOrigin("minimum").MarkBeta(),
	})
}
