...
type DaemonSetLister interface {
	...
	List(selector labels.Selector) (ret []*apiappsv1.DaemonSet, err error)
...
// List lists all DaemonSets in the indexer.
func (s *daemonSetLister) List(selector labels.Selector) (ret []*apiappsv1.DaemonSet, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*apiappsv1.DaemonSet))
	})
	return ret, err
}
...
type DaemonSetNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*apiappsv1.DaemonSet, error)
...
// List lists all DaemonSets in the indexer for a given namespace.
func (s daemonSetNamespaceLister) List(selector labels.Selector) (ret []*apiappsv1.DaemonSet, err error) {
...
// Get retrieves the DaemonSet from the indexer for a given namespace and name.
func (s daemonSetNamespaceLister) Get(name string) (*apiappsv1.DaemonSet, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(apiappsv1.Resource("daemonset"), name)
	}
	return obj.(*apiappsv1.DaemonSet), nil
}
