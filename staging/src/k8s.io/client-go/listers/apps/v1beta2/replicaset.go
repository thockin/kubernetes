...
type ReplicaSetLister interface {
	...
	List(selector labels.Selector) (ret []*apiappsv1beta2.ReplicaSet, err error)
...
// List lists all ReplicaSets in the indexer.
func (s *replicaSetLister) List(selector labels.Selector) (ret []*apiappsv1beta2.ReplicaSet, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*apiappsv1beta2.ReplicaSet))
	})
	return ret, err
}
...
type ReplicaSetNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*apiappsv1beta2.ReplicaSet, error)
...
// List lists all ReplicaSets in the indexer for a given namespace.
func (s replicaSetNamespaceLister) List(selector labels.Selector) (ret []*apiappsv1beta2.ReplicaSet, err error) {
...
// Get retrieves the ReplicaSet from the indexer for a given namespace and name.
func (s replicaSetNamespaceLister) Get(name string) (*apiappsv1beta2.ReplicaSet, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(apiappsv1beta2.Resource("replicaset"), name)
	}
	return obj.(*apiappsv1beta2.ReplicaSet), nil
}
