...
type CSIStorageCapacityLister interface {
	...
	List(selector labels.Selector) (ret []*apistoragev1.CSIStorageCapacity, err error)
...
// List lists all CSIStorageCapacities in the indexer.
func (s *cSIStorageCapacityLister) List(selector labels.Selector) (ret []*apistoragev1.CSIStorageCapacity, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*apistoragev1.CSIStorageCapacity))
	})
	return ret, err
}
...
type CSIStorageCapacityNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*apistoragev1.CSIStorageCapacity, error)
...
// List lists all CSIStorageCapacities in the indexer for a given namespace.
func (s cSIStorageCapacityNamespaceLister) List(selector labels.Selector) (ret []*apistoragev1.CSIStorageCapacity, err error) {
...
// Get retrieves the CSIStorageCapacity from the indexer for a given namespace and name.
func (s cSIStorageCapacityNamespaceLister) Get(name string) (*apistoragev1.CSIStorageCapacity, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(apistoragev1.Resource("csistoragecapacity"), name)
	}
	return obj.(*apistoragev1.CSIStorageCapacity), nil
}
