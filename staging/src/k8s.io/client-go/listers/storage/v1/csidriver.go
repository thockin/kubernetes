...
type CSIDriverLister interface {
	...
	List(selector labels.Selector) (ret []*apistoragev1.CSIDriver, err error)
	// Get retrieves the CSIDriver from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*apistoragev1.CSIDriver, error)
...
// List lists all CSIDrivers in the indexer.
func (s *cSIDriverLister) List(selector labels.Selector) (ret []*apistoragev1.CSIDriver, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*apistoragev1.CSIDriver))
	})
	return ret, err
}

// Get retrieves the CSIDriver from the index for a given name.
func (s *cSIDriverLister) Get(name string) (*apistoragev1.CSIDriver, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(apistoragev1.Resource("csidriver"), name)
	}
	return obj.(*apistoragev1.CSIDriver), nil
}
