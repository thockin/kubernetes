...
type CertificateSigningRequestLister interface {
	...
	List(selector labels.Selector) (ret []*apicertificatesv1.CertificateSigningRequest, err error)
	// Get retrieves the CertificateSigningRequest from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*apicertificatesv1.CertificateSigningRequest, error)
...
// List lists all CertificateSigningRequests in the indexer.
func (s *certificateSigningRequestLister) List(selector labels.Selector) (ret []*apicertificatesv1.CertificateSigningRequest, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*apicertificatesv1.CertificateSigningRequest))
	})
	return ret, err
}

// Get retrieves the CertificateSigningRequest from the index for a given name.
func (s *certificateSigningRequestLister) Get(name string) (*apicertificatesv1.CertificateSigningRequest, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(apicertificatesv1.Resource("certificatesigningrequest"), name)
	}
	return obj.(*apicertificatesv1.CertificateSigningRequest), nil
}
