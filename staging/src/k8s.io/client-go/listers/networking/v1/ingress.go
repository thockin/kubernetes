...
type IngressLister interface {
	...
	List(selector labels.Selector) (ret []*apinetworkingv1.Ingress, err error)
...
// List lists all Ingresses in the indexer.
func (s *ingressLister) List(selector labels.Selector) (ret []*apinetworkingv1.Ingress, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*apinetworkingv1.Ingress))
	})
	return ret, err
}
...
type IngressNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*apinetworkingv1.Ingress, error)
...
// List lists all Ingresses in the indexer for a given namespace.
func (s ingressNamespaceLister) List(selector labels.Selector) (ret []*apinetworkingv1.Ingress, err error) {
...
// Get retrieves the Ingress from the indexer for a given namespace and name.
func (s ingressNamespaceLister) Get(name string) (*apinetworkingv1.Ingress, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(apinetworkingv1.Resource("ingress"), name)
	}
	return obj.(*apinetworkingv1.Ingress), nil
}
