...
type RoleLister interface {
	...
	List(selector labels.Selector) (ret []*apirbacv1alpha1.Role, err error)
...
// List lists all Roles in the indexer.
func (s *roleLister) List(selector labels.Selector) (ret []*apirbacv1alpha1.Role, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*apirbacv1alpha1.Role))
	})
	return ret, err
}
...
type RoleNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*apirbacv1alpha1.Role, error)
...
// List lists all Roles in the indexer for a given namespace.
func (s roleNamespaceLister) List(selector labels.Selector) (ret []*apirbacv1alpha1.Role, err error) {
...
// Get retrieves the Role from the indexer for a given namespace and name.
func (s roleNamespaceLister) Get(name string) (*apirbacv1alpha1.Role, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(apirbacv1alpha1.Resource("role"), name)
	}
	return obj.(*apirbacv1alpha1.Role), nil
}
