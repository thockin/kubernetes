...
type ClusterRoleBindingLister interface {
	...
	List(selector labels.Selector) (ret []*apirbacv1beta1.ClusterRoleBinding, err error)
	// Get retrieves the ClusterRoleBinding from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*apirbacv1beta1.ClusterRoleBinding, error)
...
// List lists all ClusterRoleBindings in the indexer.
func (s *clusterRoleBindingLister) List(selector labels.Selector) (ret []*apirbacv1beta1.ClusterRoleBinding, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*apirbacv1beta1.ClusterRoleBinding))
	})
	return ret, err
}

// Get retrieves the ClusterRoleBinding from the index for a given name.
func (s *clusterRoleBindingLister) Get(name string) (*apirbacv1beta1.ClusterRoleBinding, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(apirbacv1beta1.Resource("clusterrolebinding"), name)
	}
	return obj.(*apirbacv1beta1.ClusterRoleBinding), nil
}
