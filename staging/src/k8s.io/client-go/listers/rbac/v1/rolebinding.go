...
type RoleBindingLister interface {
	...
	List(selector labels.Selector) (ret []*apirbacv1.RoleBinding, err error)
...
// List lists all RoleBindings in the indexer.
func (s *roleBindingLister) List(selector labels.Selector) (ret []*apirbacv1.RoleBinding, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*apirbacv1.RoleBinding))
	})
	return ret, err
}
...
type RoleBindingNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*apirbacv1.RoleBinding, error)
...
// List lists all RoleBindings in the indexer for a given namespace.
func (s roleBindingNamespaceLister) List(selector labels.Selector) (ret []*apirbacv1.RoleBinding, err error) {
...
// Get retrieves the RoleBinding from the indexer for a given namespace and name.
func (s roleBindingNamespaceLister) Get(name string) (*apirbacv1.RoleBinding, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(apirbacv1.Resource("rolebinding"), name)
	}
	return obj.(*apirbacv1.RoleBinding), nil
}
