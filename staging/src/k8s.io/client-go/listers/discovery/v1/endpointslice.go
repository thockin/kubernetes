...
type EndpointSliceLister interface {
	...
	List(selector labels.Selector) (ret []*apidiscoveryv1.EndpointSlice, err error)
...
// List lists all EndpointSlices in the indexer.
func (s *endpointSliceLister) List(selector labels.Selector) (ret []*apidiscoveryv1.EndpointSlice, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*apidiscoveryv1.EndpointSlice))
	})
	return ret, err
}
...
type EndpointSliceNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*apidiscoveryv1.EndpointSlice, error)
...
// List lists all EndpointSlices in the indexer for a given namespace.
func (s endpointSliceNamespaceLister) List(selector labels.Selector) (ret []*apidiscoveryv1.EndpointSlice, err error) {
...
// Get retrieves the EndpointSlice from the indexer for a given namespace and name.
func (s endpointSliceNamespaceLister) Get(name string) (*apidiscoveryv1.EndpointSlice, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(apidiscoveryv1.Resource("endpointslice"), name)
	}
	return obj.(*apidiscoveryv1.EndpointSlice), nil
}
