...
type JobLister interface {
	...
	List(selector labels.Selector) (ret []*apibatchv1.Job, err error)
...
// List lists all Jobs in the indexer.
func (s *jobLister) List(selector labels.Selector) (ret []*apibatchv1.Job, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*apibatchv1.Job))
	})
	return ret, err
}
...
type JobNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*apibatchv1.Job, error)
...
// List lists all Jobs in the indexer for a given namespace.
func (s jobNamespaceLister) List(selector labels.Selector) (ret []*apibatchv1.Job, err error) {
...
// Get retrieves the Job from the indexer for a given namespace and name.
func (s jobNamespaceLister) Get(name string) (*apibatchv1.Job, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(apibatchv1.Resource("job"), name)
	}
	return obj.(*apibatchv1.Job), nil
}
