...
type EventLister interface {
	...
	List(selector labels.Selector) (ret []*apieventsv1beta1.Event, err error)
...
// List lists all Events in the indexer.
func (s *eventLister) List(selector labels.Selector) (ret []*apieventsv1beta1.Event, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*apieventsv1beta1.Event))
	})
	return ret, err
}
...
type EventNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*apieventsv1beta1.Event, error)
...
// List lists all Events in the indexer for a given namespace.
func (s eventNamespaceLister) List(selector labels.Selector) (ret []*apieventsv1beta1.Event, err error) {
...
// Get retrieves the Event from the indexer for a given namespace and name.
func (s eventNamespaceLister) Get(name string) (*apieventsv1beta1.Event, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(apieventsv1beta1.Resource("event"), name)
	}
	return obj.(*apieventsv1beta1.Event), nil
}
