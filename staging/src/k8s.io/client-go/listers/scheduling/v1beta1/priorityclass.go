...
type PriorityClassLister interface {
	...
	List(selector labels.Selector) (ret []*apischedulingv1beta1.PriorityClass, err error)
	// Get retrieves the PriorityClass from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*apischedulingv1beta1.PriorityClass, error)
...
// List lists all PriorityClasses in the indexer.
func (s *priorityClassLister) List(selector labels.Selector) (ret []*apischedulingv1beta1.PriorityClass, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*apischedulingv1beta1.PriorityClass))
	})
	return ret, err
}

// Get retrieves the PriorityClass from the index for a given name.
func (s *priorityClassLister) Get(name string) (*apischedulingv1beta1.PriorityClass, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(apischedulingv1beta1.Resource("priorityclass"), name)
	}
	return obj.(*apischedulingv1beta1.PriorityClass), nil
}
