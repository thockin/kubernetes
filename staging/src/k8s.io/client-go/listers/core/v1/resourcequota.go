...
type ResourceQuotaLister interface {
	...
	List(selector labels.Selector) (ret []*apicorev1.ResourceQuota, err error)
...
// List lists all ResourceQuotas in the indexer.
func (s *resourceQuotaLister) List(selector labels.Selector) (ret []*apicorev1.ResourceQuota, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*apicorev1.ResourceQuota))
	})
	return ret, err
}
...
type ResourceQuotaNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*apicorev1.ResourceQuota, error)
...
// List lists all ResourceQuotas in the indexer for a given namespace.
func (s resourceQuotaNamespaceLister) List(selector labels.Selector) (ret []*apicorev1.ResourceQuota, err error) {
...
// Get retrieves the ResourceQuota from the indexer for a given namespace and name.
func (s resourceQuotaNamespaceLister) Get(name string) (*apicorev1.ResourceQuota, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(apicorev1.Resource("resourcequota"), name)
	}
	return obj.(*apicorev1.ResourceQuota), nil
}
