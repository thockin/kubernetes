...
type PodTemplateLister interface {
	...
	List(selector labels.Selector) (ret []*apicorev1.PodTemplate, err error)
...
// List lists all PodTemplates in the indexer.
func (s *podTemplateLister) List(selector labels.Selector) (ret []*apicorev1.PodTemplate, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*apicorev1.PodTemplate))
	})
	return ret, err
}
...
type PodTemplateNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*apicorev1.PodTemplate, error)
...
// List lists all PodTemplates in the indexer for a given namespace.
func (s podTemplateNamespaceLister) List(selector labels.Selector) (ret []*apicorev1.PodTemplate, err error) {
...
// Get retrieves the PodTemplate from the indexer for a given namespace and name.
func (s podTemplateNamespaceLister) Get(name string) (*apicorev1.PodTemplate, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(apicorev1.Resource("podtemplate"), name)
	}
	return obj.(*apicorev1.PodTemplate), nil
}
