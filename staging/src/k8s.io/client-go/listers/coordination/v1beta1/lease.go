...
type LeaseLister interface {
	...
	List(selector labels.Selector) (ret []*apicoordinationv1beta1.Lease, err error)
...
// List lists all Leases in the indexer.
func (s *leaseLister) List(selector labels.Selector) (ret []*apicoordinationv1beta1.Lease, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*apicoordinationv1beta1.Lease))
	})
	return ret, err
}
...
type LeaseNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*apicoordinationv1beta1.Lease, error)
...
// List lists all Leases in the indexer for a given namespace.
func (s leaseNamespaceLister) List(selector labels.Selector) (ret []*apicoordinationv1beta1.Lease, err error) {
...
// Get retrieves the Lease from the indexer for a given namespace and name.
func (s leaseNamespaceLister) Get(name string) (*apicoordinationv1beta1.Lease, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(apicoordinationv1beta1.Resource("lease"), name)
	}
	return obj.(*apicoordinationv1beta1.Lease), nil
}
