...
type PodDisruptionBudgetLister interface {
	...
	List(selector labels.Selector) (ret []*apipolicyv1beta1.PodDisruptionBudget, err error)
...
// List lists all PodDisruptionBudgets in the indexer.
func (s *podDisruptionBudgetLister) List(selector labels.Selector) (ret []*apipolicyv1beta1.PodDisruptionBudget, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*apipolicyv1beta1.PodDisruptionBudget))
	})
	return ret, err
}
...
type PodDisruptionBudgetNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*apipolicyv1beta1.PodDisruptionBudget, error)
...
// List lists all PodDisruptionBudgets in the indexer for a given namespace.
func (s podDisruptionBudgetNamespaceLister) List(selector labels.Selector) (ret []*apipolicyv1beta1.PodDisruptionBudget, err error) {
...
// Get retrieves the PodDisruptionBudget from the indexer for a given namespace and name.
func (s podDisruptionBudgetNamespaceLister) Get(name string) (*apipolicyv1beta1.PodDisruptionBudget, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(apipolicyv1beta1.Resource("poddisruptionbudget"), name)
	}
	return obj.(*apipolicyv1beta1.PodDisruptionBudget), nil
}
