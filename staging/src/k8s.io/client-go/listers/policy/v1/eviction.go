...
type EvictionLister interface {
	...
	List(selector labels.Selector) (ret []*apipolicyv1.Eviction, err error)
...
// List lists all Evictions in the indexer.
func (s *evictionLister) List(selector labels.Selector) (ret []*apipolicyv1.Eviction, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*apipolicyv1.Eviction))
	})
	return ret, err
}
...
type EvictionNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*apipolicyv1.Eviction, error)
...
// List lists all Evictions in the indexer for a given namespace.
func (s evictionNamespaceLister) List(selector labels.Selector) (ret []*apipolicyv1.Eviction, err error) {
...
// Get retrieves the Eviction from the indexer for a given namespace and name.
func (s evictionNamespaceLister) Get(name string) (*apipolicyv1.Eviction, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(apipolicyv1.Resource("eviction"), name)
	}
	return obj.(*apipolicyv1.Eviction), nil
}
