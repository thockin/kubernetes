...
var (
	protocolTCP  = apicommon.ProtocolTCP
	protocolUDP  = apicommon.ProtocolUDP
	protocolSCTP = apicommon.ProtocolSCTP
)
...
var _ = common.SIGDescribe("Netpol", func() {
	...
	ginkgo.Context("NetworkPolicy between server and client", func() {
		...
		ginkgo.It("should support a 'default-deny-ingress' policy [Feature:NetworkPolicy]", func() {
			...
			protocols := []apicommon.Protocol{protocolTCP}
			ports := []int32{80}
			...
			ValidateOrFail(k8s, model, &TestCase{ToPort: 80, Protocol: apicommon.ProtocolTCP, Reachability: reachability})
		})
		...
		ginkgo.It("should enforce policy based on Ports [Feature:NetworkPolicy]", func() {
			...
			ValidateOrFail(k8s, model, &TestCase{ToPort: 81, Protocol: apicommon.ProtocolTCP, Reachability: reachability})
		})
		...
		ginkgo.It("should enforce multiple, stacked policies with overlapping podSelectors [Feature:NetworkPolicy]", func() {
			...
			ValidateOrFail(k8s, model, &TestCase{ToPort: 81, Protocol: apicommon.ProtocolTCP, Reachability: reachabilityALLOW})
			...
			ValidateOrFail(k8s, model, &TestCase{ToPort: 80, Protocol: apicommon.ProtocolTCP, Reachability: reachabilityDENY})
			...
			ValidateOrFail(k8s, model, &TestCase{ToPort: 80, Protocol: apicommon.ProtocolTCP, Reachability: reachabilityALLOW})
		})
		...
		ginkgo.It("should allow ingress access on one named port [Feature:NetworkPolicy]", func() {
			...
			ValidateOrFail(k8s, model, &TestCase{ToPort: 81, Protocol: apicommon.ProtocolTCP, Reachability: reachabilityPort81})
			...
			ValidateOrFail(k8s, model, &TestCase{ToPort: 80, Protocol: apicommon.ProtocolTCP, Reachability: reachabilityPort80})
		})
		...
		ginkgo.It("should allow ingress access from namespace on one named port [Feature:NetworkPolicy]", func() {
			...
			ValidateOrFail(k8s, model, &TestCase{ToPort: 81, Protocol: apicommon.ProtocolTCP, Reachability: reachabilityFAIL})
		})
		...
		ginkgo.It("should enforce updated policy [Feature:NetworkPolicy]", func() {
			...
			ValidateOrFail(k8s, model, &TestCase{ToPort: 81, Protocol: apicommon.ProtocolTCP, Reachability: reachabilityDeny})
		})
		...
		ginkgo.It("should allow ingress access from updated namespace [Feature:NetworkPolicy]", func() {
			...
			ValidateOrFail(k8s, model, &TestCase{ToPort: 80, Protocol: apicommon.ProtocolTCP, Reachability: reachabilityWithLabel})
		})
		...
		ginkgo.It("should deny ingress access to updated pod [Feature:NetworkPolicy]", func() {
			...
			ValidateOrFail(k8s, model, &TestCase{ToPort: 80, Protocol: apicommon.ProtocolTCP, Reachability: reachabilityIsolated})
		})
		...
		ginkgo.It("should enforce ingress policy allowing any port traffic to a server on a specific protocol [Feature:NetworkPolicy] [Feature:UDP]", func() {
			nsX, _, _, k8s := getK8sNamespaces(f)
			protocols := []apicommon.Protocol{protocolTCP, protocolUDP}
			ports := []int32{80}
			model = initializeResourcesByFixedNS(f, protocols, ports)
			ingressRule := networkingv1.NetworkPolicyIngressRule{}
			...
			ValidateOrFail(k8s, model, &TestCase{ToPort: 80, Protocol: apicommon.ProtocolTCP, Reachability: reachabilityTCP})

			reachabilityUDP := NewReachability(model.AllPods(), true)
			reachabilityUDP.ExpectPeer(&Peer{}, &Peer{Namespace: nsX, Pod: "a"}, false)
			ValidateOrFail(k8s, model, &TestCase{ToPort: 80, Protocol: apicommon.ProtocolUDP, Reachability: reachabilityUDP})
		})
		...
		ginkgo.It("should enforce multiple ingress policies with ingress allow-all policy taking precedence [Feature:NetworkPolicy]", func() {
			...
			ValidateOrFail(k8s, model, &TestCase{ToPort: 81, Protocol: apicommon.ProtocolTCP, Reachability: reachabilityAll})
		})
		...
		ginkgo.It("should stop enforcing policies after they are deleted [Feature:NetworkPolicy]", func() {
			...
			reachabilityAll := NewReachability(model.AllPods(), true)
			ValidateOrFail(k8s, model, &TestCase{ToPort: 80, Protocol: apicommon.ProtocolTCP, Reachability: reachabilityAll})
		})
		...
		ginkgo.It("should enforce policies to check ingress and egress policies can be controlled independently based on PodSelector [Feature:NetworkPolicy]", func() {
			...
			*/
			...
			ValidateOrFail(k8s, model, &TestCase{ToPort: 80, Protocol: apicommon.ProtocolTCP, Reachability: allowEgressReachability})
			...
			ValidateOrFail(k8s, model, &TestCase{ToPort: 80, Protocol: apicommon.ProtocolTCP, Reachability: denyIngressToXReachability})
		})
...
var _ = common.SIGDescribe("Netpol [LinuxOnly]", func() {
	...
	ginkgo.Context("NetworkPolicy between server and client using UDP", func() {
		...
		ginkgo.It("should support a 'default-deny-ingress' policy [Feature:NetworkPolicy]", func() {
			nsX, _, _, k8s := getK8sNamespaces(f)
			protocols := []apicommon.Protocol{protocolUDP}
			ports := []int32{80}
			model = initializeResourcesByFixedNS(f, protocols, ports)
			policy := GenNetworkPolicyWithNameAndPodSelector("deny-all", metav1.LabelSelector{}, SetSpecIngressRules())
			CreatePolicy(k8s, policy, nsX)
			...
			ValidateOrFail(k8s, model, &TestCase{ToPort: 80, Protocol: apicommon.ProtocolUDP, Reachability: reachability})
		})
		...
		ginkgo.It("should enforce policy based on Ports [Feature:NetworkPolicy]", func() {
			...
			ValidateOrFail(k8s, model, &TestCase{ToPort: 81, Protocol: apicommon.ProtocolUDP, Reachability: reachability})
		})
...
var _ = common.SIGDescribe("Netpol [Feature:SCTPConnectivity][LinuxOnly][Disruptive]", func() {
	...
	ginkgo.Context("NetworkPolicy between server and client using SCTP", func() {
		...
		ginkgo.It("should support a 'default-deny-ingress' policy [Feature:NetworkPolicy]", func() {
			nsX, _, _, k8s := getK8sNamespaces(f)
			protocols := []apicommon.Protocol{protocolSCTP}
			ports := []int32{80}
			model = initializeResourcesByFixedNS(f, protocols, ports)
			policy := GenNetworkPolicyWithNameAndPodSelector("deny-all", metav1.LabelSelector{}, SetSpecIngressRules())
			CreatePolicy(k8s, policy, nsX)
			...
			ValidateOrFail(k8s, model, &TestCase{ToPort: 80, Protocol: apicommon.ProtocolSCTP, Reachability: reachability})
		})
		...
		ginkgo.It("should enforce policy based on Ports [Feature:NetworkPolicy]", func() {
			...
			ValidateOrFail(k8s, model, &TestCase{ToPort: 81, Protocol: apicommon.ProtocolSCTP, Reachability: reachability})
		})
...
func defaultModel(namespaces []string, dnsDomain string, protocols []apicommon.Protocol, ports []int32) *Model {
...
func initializeResourcesByFixedNS(f *framework.Framework, protocols []apicommon.Protocol, ports []int32) *Model {
	if useFixedNamespaces {
...
func initializeResources(f *framework.Framework, protocols []apicommon.Protocol, ports []int32) (*Model, error) {
	k8s := newKubeManager(f)
