...
func ValidateLabelName(labelName string, fldPath *field.Path) field.ErrorList {
	...
	if ok, errs := validation.IsQualifiedName(labelName); !ok {
		for i := range errs {
			allErrs = append(allErrs, fldPath.InvalidError(labelName, errs[i]))
		}
	}
	return allErrs
}
...
func ValidateLabels(labels map[string]string, fldPath *field.Path) field.ErrorList {
	...
	for k, v := range labels {
		...
		if ok, errs := validation.IsLabelValue(v); !ok {
			for i := range errs {
				allErrs = append(allErrs, fldPath.InvalidError(v, errs[i]))
			}
		}
	}
	return allErrs
...
func ValidateAnnotations(annotations map[string]string, fldPath *field.Path) field.ErrorList {
	...
	for k, v := range annotations {
		if ok, errs := validation.IsQualifiedName(strings.ToLower(k)); !ok {
			for i := range errs {
				allErrs = append(allErrs, fldPath.InvalidError(k, errs[i]))
			}
		}
		totalSize += (int64)(len(k)) + (int64)(len(v))
	}
	if totalSize > (int64)(totalAnnotationSizeLimitB) {
		allErrs = append(allErrs, fldPath.TooLongError("", totalAnnotationSizeLimitB))
	}
...
// name will have a value appended to it.  If the bool result is false, the
// string-slice result contains descriptions of individual characteristics of
// the value that were not valid.
...
func ValidateLessThan(max, value int64, fldPath *field.Path) field.ErrorList {
	...
	if ok, msgs := validation.IsLessThan(max, value); !ok {
		for i := range msgs {
			allErrs = append(allErrs, fldPath.InvalidError(value, msgs[i]))
		}
	}
	return allErrs
}
...
func ValidateNonnegativeQuantity(value resource.Quantity, fldPath *field.Path) field.ErrorList {
	...
	if ok, msgs := validation.IsGreaterThanOrEqual(0, int64(cmp)); !ok {
		for i := range msgs {
			allErrs = append(allErrs, fldPath.InvalidError(value.String(), msgs[i]))
		}
	}
	return allErrs
}
...
func ValidateImmutableField(newVal, oldVal interface{}, fldPath *field.Path) field.ErrorList {
	...
	if !api.Semantic.DeepEqual(oldVal, newVal) {
		allErrs = append(allErrs, fldPath.InvalidError(newVal, fieldImmutableErrorMsg))
	}
	return allErrs
}
...
func ValidateObjectMeta(meta *api.ObjectMeta, requiresNamespace bool, nameFn ValidateNameFunc, fldPath *field.Path) field.ErrorList {
	...
	if len(meta.GenerateName) != 0 {
		if ok, errs := nameFn(meta.GenerateName, true); !ok {
			for i := range errs {
				allErrs = append(allErrs, fldPath.Child("generateName").InvalidError(meta.GenerateName, errs[i]))
			}
		}
	}
	...
	if len(meta.Name) == 0 {
		allErrs = append(allErrs, fldPath.Child("name").RequiredError("name or generateName is required"))
	} else {
		if ok, errs := nameFn(meta.Name, false); !ok {
			for i := range errs {
				allErrs = append(allErrs, fldPath.Child("name").InvalidError(meta.Name, errs[i]))
			}
		}
	}
	if requiresNamespace {
		if len(meta.Namespace) == 0 {
			allErrs = append(allErrs, fldPath.Child("namespace").RequiredError(""))
		} else if ok, errs := ValidateNamespaceName(meta.Namespace, false); !ok {
			for i := range errs {
				allErrs = append(allErrs, fldPath.Child("namespace").InvalidError(meta.Namespace, errs[i]))
			}
		}
	} else {
		if len(meta.Namespace) != 0 {
			allErrs = append(allErrs, fldPath.Child("namespace").ForbiddenError("not allowed on this type"))
		}
	}
...
func ValidateObjectMetaUpdate(newMeta, oldMeta *api.ObjectMeta, fldPath *field.Path) field.ErrorList {
	...
	if !RepairMalformedUpdates && newMeta.UID != oldMeta.UID {
		allErrs = append(allErrs, fldPath.Child("uid").InvalidError(newMeta.UID, "field is immutable"))
	}
	...
	if newMeta.DeletionGracePeriodSeconds != nil && oldMeta.DeletionGracePeriodSeconds != nil && *newMeta.DeletionGracePeriodSeconds != *oldMeta.DeletionGracePeriodSeconds {
		allErrs = append(allErrs, fldPath.Child("deletionGracePeriodSeconds").InvalidError(newMeta.DeletionGracePeriodSeconds, "field is immutable; may only be changed via deletion"))
	}

	// Reject updates that don't specify a resource version
	if len(newMeta.ResourceVersion) == 0 {
		allErrs = append(allErrs, fldPath.Child("resourceVersion").InvalidError(newMeta.ResourceVersion, "must be specified for an update"))
	}
...
func validateVolumes(volumes []api.Volume, fldPath *field.Path) (sets.String, field.ErrorList) {
	...
	for i, vol := range volumes {
		...
		if len(vol.Name) == 0 {
			el = append(el, idxPath.Child("name").RequiredError(""))
		} else if ok, errs := validation.IsDNS1123Label(vol.Name); !ok {
			for i := range errs {
				el = append(el, idxPath.Child("name").InvalidError(vol.Name, errs[i]))
			}
		} else if allNames.Has(vol.Name) {
			el = append(el, idxPath.Child("name").DuplicateError(vol.Name))
		}
		if len(el) == 0 {
...
func validateVolumeSource(source *api.VolumeSource, fldPath *field.Path) field.ErrorList {
	...
	if source.HostPath != nil {
		if numVolumes > 0 {
			allErrs = append(allErrs, fldPath.Child("hostPath").ForbiddenError("may not specify more than 1 volume type"))
		} else {
			numVolumes++
	...
	if source.GitRepo != nil {
		if numVolumes > 0 {
			allErrs = append(allErrs, fldPath.Child("gitRepo").ForbiddenError("may not specify more than 1 volume type"))
		} else {
			numVolumes++
	...
	if source.GCEPersistentDisk != nil {
		if numVolumes > 0 {
			allErrs = append(allErrs, fldPath.Child("gcePersistentDisk").ForbiddenError("may not specify more than 1 volume type"))
		} else {
			numVolumes++
	...
	if source.AWSElasticBlockStore != nil {
		if numVolumes > 0 {
			allErrs = append(allErrs, fldPath.Child("awsElasticBlockStore").ForbiddenError("may not specify more than 1 volume type"))
		} else {
			numVolumes++
	...
	if source.Secret != nil {
		if numVolumes > 0 {
			allErrs = append(allErrs, fldPath.Child("secret").ForbiddenError("may not specify more than 1 volume type"))
		} else {
			numVolumes++
	...
	if source.NFS != nil {
		if numVolumes > 0 {
			allErrs = append(allErrs, fldPath.Child("nfs").ForbiddenError("may not specify more than 1 volume type"))
		} else {
			numVolumes++
	...
	if source.ISCSI != nil {
		if numVolumes > 0 {
			allErrs = append(allErrs, fldPath.Child("iscsi").ForbiddenError("may not specify more than 1 volume type"))
		} else {
			numVolumes++
	...
	if source.Glusterfs != nil {
		if numVolumes > 0 {
			allErrs = append(allErrs, fldPath.Child("glusterfs").ForbiddenError("may not specify more than 1 volume type"))
		} else {
			numVolumes++
	...
	if source.Flocker != nil {
		if numVolumes > 0 {
			allErrs = append(allErrs, fldPath.Child("flocker").ForbiddenError("may not specify more than 1 volume type"))
		} else {
			numVolumes++
	...
	if source.PersistentVolumeClaim != nil {
		if numVolumes > 0 {
			allErrs = append(allErrs, fldPath.Child("persistentVolumeClaim").ForbiddenError("may not specify more than 1 volume type"))
		} else {
			numVolumes++
	...
	if source.RBD != nil {
		if numVolumes > 0 {
			allErrs = append(allErrs, fldPath.Child("rbd").ForbiddenError("may not specify more than 1 volume type"))
		} else {
			numVolumes++
	...
	if source.Cinder != nil {
		if numVolumes > 0 {
			allErrs = append(allErrs, fldPath.Child("cinder").ForbiddenError("may not specify more than 1 volume type"))
		} else {
			numVolumes++
	...
	if source.CephFS != nil {
		if numVolumes > 0 {
			allErrs = append(allErrs, fldPath.Child("cephFS").ForbiddenError("may not specify more than 1 volume type"))
		} else {
			numVolumes++
	...
	if source.DownwardAPI != nil {
		if numVolumes > 0 {
			allErrs = append(allErrs, fldPath.Child("downwarAPI").ForbiddenError("may not specify more than 1 volume type"))
		} else {
			numVolumes++
	...
	if source.FC != nil {
		if numVolumes > 0 {
			allErrs = append(allErrs, fldPath.Child("fc").ForbiddenError("may not specify more than 1 volume type"))
		} else {
			numVolumes++
	...
	if source.FlexVolume != nil {
		if numVolumes > 0 {
			allErrs = append(allErrs, fldPath.Child("flexVolume").ForbiddenError("may not specifiy more than 1 volume type"))
		} else {
			numVolumes++
	...
	if source.ConfigMap != nil {
		if numVolumes > 0 {
			allErrs = append(allErrs, fldPath.Child("configMap").ForbiddenError("may not specifiy more than 1 volume type"))
		} else {
			numVolumes++
	...
	if numVolumes == 0 {
		allErrs = append(allErrs, fldPath.RequiredError("must specify a volume type"))
	}
...
func validateHostPathVolumeSource(hostPath *api.HostPathVolumeSource, fldPath *field.Path) field.ErrorList {
	...
	if len(hostPath.Path) == 0 {
		allErrs = append(allErrs, fldPath.Child("path").RequiredError(""))
	}
	return allErrs
}
...
func validateGitRepoVolumeSource(gitRepo *api.GitRepoVolumeSource, fldPath *field.Path) field.ErrorList {
	...
	if len(gitRepo.Repository) == 0 {
		allErrs = append(allErrs, fldPath.Child("repository").RequiredError(""))
	}
...
func validateISCSIVolumeSource(iscsi *api.ISCSIVolumeSource, fldPath *field.Path) field.ErrorList {
	...
	if len(iscsi.TargetPortal) == 0 {
		allErrs = append(allErrs, fldPath.Child("targetPortal").RequiredError(""))
	}
	if len(iscsi.IQN) == 0 {
		allErrs = append(allErrs, fldPath.Child("iqn").RequiredError(""))
	}
	if ok, msgs := validation.IsBetweenInclusive(0, 255, int64(iscsi.Lun)); !ok {
		for i := range msgs {
			allErrs = append(allErrs, fldPath.Child("lun").InvalidError(iscsi.Lun, msgs[i]))
		}
	}
	return allErrs
}
...
func validateFCVolumeSource(fc *api.FCVolumeSource, fldPath *field.Path) field.ErrorList {
	...
	if len(fc.TargetWWNs) < 1 {
		allErrs = append(allErrs, fldPath.Child("targetWWNs").RequiredError(""))
	}

	if fc.Lun == nil {
		allErrs = append(allErrs, fldPath.Child("lun").RequiredError(""))
	} else if ok, msgs := validation.IsBetweenInclusive(0, 255, int64(*fc.Lun)); !ok {
		for i := range msgs {
			allErrs = append(allErrs, fldPath.Child("lun").InvalidError(*fc.Lun, msgs[i]))
		}
	}
	return allErrs
}
...
func validateGCEPersistentDiskVolumeSource(pd *api.GCEPersistentDiskVolumeSource, fldPath *field.Path) field.ErrorList {
	allErrs := field.ErrorList{}
	if len(pd.PDName) == 0 {
		allErrs = append(allErrs, fldPath.Child("pdName").RequiredError(""))
	}
	if pd.Partition > 0 {
		if ok, msgs := validation.IsBetweenInclusive(1, 255, int64(pd.Partition)); !ok {
			for i := range msgs {
				allErrs = append(allErrs, fldPath.Child("partition").InvalidError(pd.Partition, msgs[i]))
			}
		}
	}
	return allErrs
...
func validateAWSElasticBlockStoreVolumeSource(ebs *api.AWSElasticBlockStoreVolumeSource, fldPath *field.Path) field.ErrorList {
	...
	if len(ebs.VolumeID) == 0 {
		allErrs = append(allErrs, fldPath.Child("volumeID").RequiredError(""))
	}
	if ebs.Partition > 0 {
		if ok, msgs := validation.IsBetweenInclusive(1, 255, int64(ebs.Partition)); !ok {
			for i := range msgs {
				allErrs = append(allErrs, fldPath.Child("partition").InvalidError(ebs.Partition, msgs[i]))
			}
		}
	}
	return allErrs
...
func validateSecretVolumeSource(secretSource *api.SecretVolumeSource, fldPath *field.Path) field.ErrorList {
	...
	if len(secretSource.SecretName) == 0 {
		allErrs = append(allErrs, fldPath.Child("secretName").RequiredError(""))
	}
	return allErrs
}
...
func validatePersistentClaimVolumeSource(claim *api.PersistentVolumeClaimVolumeSource, fldPath *field.Path) field.ErrorList {
	...
	if len(claim.ClaimName) == 0 {
		allErrs = append(allErrs, fldPath.Child("claimName").RequiredError(""))
	}
	return allErrs
}
...
func validateNFSVolumeSource(nfs *api.NFSVolumeSource, fldPath *field.Path) field.ErrorList {
	allErrs := field.ErrorList{}
	if len(nfs.Server) == 0 {
		allErrs = append(allErrs, fldPath.Child("server").RequiredError(""))
	}
	if len(nfs.Path) == 0 {
		allErrs = append(allErrs, fldPath.Child("path").RequiredError(""))
	}
	if !path.IsAbs(nfs.Path) {
		allErrs = append(allErrs, fldPath.Child("path").InvalidError(nfs.Path, "must be an absolute path"))
	}
	return allErrs
}
...
func validateGlusterfs(glusterfs *api.GlusterfsVolumeSource, fldPath *field.Path) field.ErrorList {
	...
	if len(glusterfs.EndpointsName) == 0 {
		allErrs = append(allErrs, fldPath.Child("endpoints").RequiredError(""))
	}
...
func validateFlockerVolumeSource(flocker *api.FlockerVolumeSource, fldPath *field.Path) field.ErrorList {
	...
	if len(flocker.DatasetName) == 0 {
		allErrs = append(allErrs, fldPath.Child("datasetName").RequiredError(""))
	}
	if strings.Contains(flocker.DatasetName, "/") {
		allErrs = append(allErrs, fldPath.Child("datasetName").InvalidError(flocker.DatasetName, "must not contain '/'"))
	}
	return allErrs
}
...
func validateVolumeSourcePath(targetPath string, fldPath *field.Path) field.ErrorList {
	...
	if path.IsAbs(targetPath) {
		allErrs = append(allErrs, fldPath.InvalidError(targetPath, "must be a relative path"))
	}
	...
	for _, item := range items {
		if item == ".." {
			allErrs = append(allErrs, fldPath.InvalidError(targetPath, "must not contain '..'"))
		}
	}
	if strings.HasPrefix(items[0], "..") && len(items[0]) > 2 {
		allErrs = append(allErrs, fldPath.InvalidError(targetPath, "must not start with '..'"))
	}
	return allErrs
}
...
func validateRBDVolumeSource(rbd *api.RBDVolumeSource, fldPath *field.Path) field.ErrorList {
	...
	if len(rbd.CephMonitors) == 0 {
		allErrs = append(allErrs, fldPath.Child("monitors").RequiredError(""))
	}
	if len(rbd.RBDImage) == 0 {
		allErrs = append(allErrs, fldPath.Child("image").RequiredError(""))
	}
	return allErrs
}
...
func validateFlexVolumeSource(fv *api.FlexVolumeSource, fldPath *field.Path) field.ErrorList {
	allErrs := field.ErrorList{}
	if len(fv.Driver) == 0 {
		allErrs = append(allErrs, fldPath.Child("driver").RequiredError(""))
	}
	return allErrs
}
...
func validateAzureFile(azure *api.AzureFileVolumeSource, fldPath *field.Path) field.ErrorList {
	...
	if azure.ShareName == "" {
		allErrs = append(allErrs, fldPath.Child("shareName").RequiredError(""))
	}
	return allErrs
}
...
func ValidatePersistentVolume(pv *api.PersistentVolume) field.ErrorList {
	...
	if len(pv.Spec.AccessModes) == 0 {
		allErrs = append(allErrs, specPath.Child("accessModes").RequiredError(""))
	}
	for _, mode := range pv.Spec.AccessModes {
		if !supportedAccessModes.Has(string(mode)) {
			allErrs = append(allErrs, specPath.Child("accessModes").NotSupportedError(mode, supportedAccessModes.List()))
		}
	}

	if len(pv.Spec.Capacity) == 0 {
		allErrs = append(allErrs, specPath.Child("capacity").RequiredError(""))
	}

	if _, ok := pv.Spec.Capacity[api.ResourceStorage]; !ok || len(pv.Spec.Capacity) > 1 {
		allErrs = append(allErrs, specPath.Child("capacity").NotSupportedError(pv.Spec.Capacity, []string{string(api.ResourceStorage)}))
	}
	...
	if pv.Spec.HostPath != nil {
		if numVolumes > 0 {
			allErrs = append(allErrs, specPath.Child("hostPath").ForbiddenError("may not specify more than 1 volume type"))
		} else {
			numVolumes++
	...
	if pv.Spec.GCEPersistentDisk != nil {
		if numVolumes > 0 {
			allErrs = append(allErrs, specPath.Child("gcePersistentDisk").ForbiddenError("may not specify more than 1 volume type"))
		} else {
			numVolumes++
	...
	if pv.Spec.AWSElasticBlockStore != nil {
		if numVolumes > 0 {
			allErrs = append(allErrs, specPath.Child("awsElasticBlockStore").ForbiddenError("may not specify more than 1 volume type"))
		} else {
			numVolumes++
	...
	if pv.Spec.Glusterfs != nil {
		if numVolumes > 0 {
			allErrs = append(allErrs, specPath.Child("glusterfs").ForbiddenError("may not specify more than 1 volume type"))
		} else {
			numVolumes++
	...
	if pv.Spec.Flocker != nil {
		if numVolumes > 0 {
			allErrs = append(allErrs, specPath.Child("flocker").ForbiddenError("may not specify more than 1 volume type"))
		} else {
			numVolumes++
	...
	if pv.Spec.NFS != nil {
		if numVolumes > 0 {
			allErrs = append(allErrs, specPath.Child("nfs").ForbiddenError("may not specify more than 1 volume type"))
		} else {
			numVolumes++
	...
	if pv.Spec.RBD != nil {
		if numVolumes > 0 {
			allErrs = append(allErrs, specPath.Child("rbd").ForbiddenError("may not specify more than 1 volume type"))
		} else {
			numVolumes++
	...
	if pv.Spec.CephFS != nil {
		if numVolumes > 0 {
			allErrs = append(allErrs, specPath.Child("cephFS").ForbiddenError("may not specify more than 1 volume type"))
		} else {
			numVolumes++
	...
	if pv.Spec.ISCSI != nil {
		if numVolumes > 0 {
			allErrs = append(allErrs, specPath.Child("iscsi").ForbiddenError("may not specify more than 1 volume type"))
		} else {
			numVolumes++
	...
	if pv.Spec.Cinder != nil {
		if numVolumes > 0 {
			allErrs = append(allErrs, specPath.Child("cinder").ForbiddenError("may not specify more than 1 volume type"))
		} else {
			numVolumes++
	...
	if pv.Spec.FC != nil {
		if numVolumes > 0 {
			allErrs = append(allErrs, specPath.Child("fc").ForbiddenError("may not specify more than 1 volume type"))
		} else {
			numVolumes++
	...
	if numVolumes == 0 {
		allErrs = append(allErrs, specPath.RequiredError("must specify a volume type"))
	}
	return allErrs
}
...
func ValidatePersistentVolumeStatusUpdate(newPv, oldPv *api.PersistentVolume) field.ErrorList {
	...
	if len(newPv.ResourceVersion) == 0 {
		allErrs = append(allErrs, field.NewPath("resourceVersion").RequiredError(""))
	}
...
func ValidatePersistentVolumeClaim(pvc *api.PersistentVolumeClaim) field.ErrorList {
	...
	if len(pvc.Spec.AccessModes) == 0 {
		allErrs = append(allErrs, specPath.Child("accessModes").RequiredError("at least 1 accessMode is required"))
	}
	...
	if _, ok := pvc.Spec.Resources.Requests[api.ResourceStorage]; !ok {
		allErrs = append(allErrs, specPath.Child("resources").Key(string(api.ResourceStorage)).RequiredError(""))
	}
	return allErrs
}
...
func ValidatePersistentVolumeClaimStatusUpdate(newPvc, oldPvc *api.PersistentVolumeClaim) field.ErrorList {
	...
	if len(newPvc.Spec.AccessModes) == 0 {
		allErrs = append(allErrs, field.NewPath("Spec", "accessModes").RequiredError(""))
	}
	...
	for i, port := range ports {
		...
		if len(port.Name) > 0 {
			if ok, msgs := validation.IsPortName(port.Name); !ok {
				for i = range msgs {
					allErrs = append(allErrs, idxPath.Child("name").InvalidError(port.Name, msgs[i]))
				}
			} else if allNames.Has(port.Name) {
				allErrs = append(allErrs, idxPath.Child("name").DuplicateError(port.Name))
			} else {
				allNames.Insert(port.Name)
			}
		}
		if port.ContainerPort == 0 {
			allErrs = append(allErrs, idxPath.Child("containerPort").RequiredError(""))
		} else if ok, msgs := validation.IsPortNum(port.ContainerPort); !ok {
			for i := range msgs {
				allErrs = append(allErrs, idxPath.Child("containerPort").InvalidError(port.ContainerPort, msgs[i]))
			}
		}
		if port.HostPort != 0 {
			if ok, msgs := validation.IsPortNum(port.HostPort); !ok {
				for i := range msgs {
					allErrs = append(allErrs, idxPath.Child("hostPort").InvalidError(port.HostPort, msgs[i]))
				}
			}
		}
		if len(port.Protocol) == 0 {
			allErrs = append(allErrs, idxPath.Child("protocol").RequiredError(""))
		} else if !supportedPortProtocols.Has(string(port.Protocol)) {
			allErrs = append(allErrs, idxPath.Child("protocol").NotSupportedError(port.Protocol, supportedPortProtocols.List()))
		}
	}
	return allErrs
}
...
func validateEnv(vars []api.EnvVar, fldPath *field.Path) field.ErrorList {
	...
	for i, ev := range vars {
		idxPath := fldPath.Index(i)
		if len(ev.Name) == 0 {
			allErrs = append(allErrs, idxPath.Child("name").RequiredError(""))
		} else if ok, msgs := validation.IsCIdentifier(ev.Name); !ok {
			for i := range msgs {
				allErrs = append(allErrs, idxPath.Child("name").InvalidError(ev.Name, msgs[i]))
			}
		}
...
func validateEnvVarValueFrom(ev api.EnvVar, fldPath *field.Path) field.ErrorList {
	...
	if len(ev.Value) != 0 {
		if numSources != 0 {
			allErrs = append(allErrs, fldPath.InvalidError("", "may not be specified when `value` is not empty"))
		}
	} else if numSources != 1 {
		allErrs = append(allErrs, fldPath.InvalidError("", "may not have more than one field specified at a time"))
	}
...
func validateObjectFieldSelector(fs *api.ObjectFieldSelector, expressions *sets.String, fldPath *field.Path) field.ErrorList {
	...
	if len(fs.APIVersion) == 0 {
		allErrs = append(allErrs, fldPath.Child("apiVersion").RequiredError(""))
	} else if len(fs.FieldPath) == 0 {
		allErrs = append(allErrs, fldPath.Child("fieldPath").RequiredError(""))
	} else {
		internalFieldPath, _, err := api.Scheme.ConvertFieldLabel(fs.APIVersion, "Pod", fs.FieldPath, "")
		if err != nil {
			allErrs = append(allErrs, fldPath.Child("fieldPath").InvalidError(fs.FieldPath, fmt.Sprintf("error converting fieldPath: %v", err)))
		} else if !expressions.Has(internalFieldPath) {
			allErrs = append(allErrs, fldPath.Child("fieldPath").NotSupportedError(internalFieldPath, expressions.List()))
		}
	}
...
func validateConfigMapKeySelector(s *api.ConfigMapKeySelector, fldPath *field.Path) field.ErrorList {
	...
	if len(s.Key) == 0 {
		allErrs = append(allErrs, fldPath.Child("key").RequiredError(""))
	} else if !IsSecretKey(s.Key) {
		allErrs = append(allErrs, fldPath.Child("key").InvalidError(s.Key, fmt.Sprintf("must have at most %d characters and match regex %s", validation.DNS1123SubdomainMaxLength, SecretKeyFmt)))
	}
...
func validateVolumeMounts(mounts []api.VolumeMount, volumes sets.String, fldPath *field.Path) field.ErrorList {
	...
	for i, mnt := range mounts {
		...
		if len(mnt.Name) == 0 {
			...
			allErrs = append(allErrs, idxPath.Child("name").NotFoundError(mnt.Name))
		}
		if len(mnt.MountPath) == 0 {
			allErrs = append(allErrs, idxPath.Child("mountPath").RequiredError(""))
		}
	}
	return allErrs
}
	...
	for ci, ctr := range containers {
		...
		for pi := range ctr.Ports {
			...
			if accumulator.Has(str) {
				allErrs = append(allErrs, idxPath.Child("hostPort").DuplicateError(str))
			} else {
...
func validateExecAction(exec *api.ExecAction, fldPath *field.Path) field.ErrorList {
	...
	if len(exec.Command) == 0 {
		allErrors = append(allErrors, fldPath.Child("command").RequiredError(""))
	}
	return allErrors
}
...
func validateHTTPGetAction(http *api.HTTPGetAction, fldPath *field.Path) field.ErrorList {
	allErrors := field.ErrorList{}
	if len(http.Path) == 0 {
		allErrors = append(allErrors, fldPath.Child("path").RequiredError(""))
	}
	...
	if !supportedHTTPSchemes.Has(string(http.Scheme)) {
		allErrors = append(allErrors, fldPath.Child("scheme").NotSupportedError(http.Scheme, supportedHTTPSchemes.List()))
	}
	for _, header := range http.HTTPHeaders {
		if ok, msgs := validation.IsHTTPHeaderName(header.Name); !ok {
			for i := range msgs {
				allErrors = append(allErrors, fldPath.Child("httpHeaders").InvalidError(header.Name, msgs[i]))
			}
		}
	}
	return allErrors
...
func ValidatePortNumOrName(port intstr.IntOrString, fldPath *field.Path) field.ErrorList {
	...
	if port.Type == intstr.Int {
		if ok, msgs := validation.IsPortNum(port.IntValue()); !ok {
			for i := range msgs {
				allErrors = append(allErrors, fldPath.InvalidError(port.IntValue(), msgs[i]))
			}
		}
	} else if port.Type == intstr.String {
		if ok, msgs := validation.IsPortName(port.StrVal); !ok {
			for i := range msgs {
				allErrors = append(allErrors, fldPath.InvalidError(port.StrVal, msgs[i]))
			}
		}
	} else {
		allErrors = append(allErrors, fldPath.InternalError(fmt.Errorf("unknown type: %v", port.Type)))
	}
	return allErrors
}
...
func validateHandler(handler *api.Handler, fldPath *field.Path) field.ErrorList {
	...
	if handler.Exec != nil {
		if numHandlers > 0 {
			allErrors = append(allErrors, fldPath.Child("exec").ForbiddenError("may not specify more than 1 handler type"))
		} else {
			numHandlers++
	...
	if handler.HTTPGet != nil {
		if numHandlers > 0 {
			allErrors = append(allErrors, fldPath.Child("httpGet").ForbiddenError("may not specify more than 1 handler type"))
		} else {
			numHandlers++
	...
	if handler.TCPSocket != nil {
		if numHandlers > 0 {
			allErrors = append(allErrors, fldPath.Child("tcpSocket").ForbiddenError("may not specify more than 1 handler type"))
		} else {
			numHandlers++
	...
	if numHandlers == 0 {
		allErrors = append(allErrors, fldPath.RequiredError("must specify a handler type"))
	}
	return allErrors
}
...
func validatePullPolicy(policy api.PullPolicy, fldPath *field.Path) field.ErrorList {
	...
	case api.PullAlways, api.PullIfNotPresent, api.PullNever:
		break
	case "":
		allErrors = append(allErrors, fldPath.RequiredError(""))
	default:
		allErrors = append(allErrors, fldPath.NotSupportedError(policy, supportedPullPolicies.List()))
	}
...
func validateContainers(containers []api.Container, volumes sets.String, fldPath *field.Path) field.ErrorList {
	...
	for i, ctr := range containers {
		...
		if len(ctr.Name) == 0 {
			...
			for i := range errs {
				allErrs = append(allErrs, idxPath.Child("name").InvalidError(ctr.Name, errs[i]))
			}
		} else if allNames.Has(ctr.Name) {
			allErrs = append(allErrs, idxPath.Child("name").DuplicateError(ctr.Name))
		} else {
			allNames.Insert(ctr.Name)
		}
		if len(ctr.Image) == 0 {
			allErrs = append(allErrs, idxPath.Child("image").RequiredError(""))
		}
		...
		if ctr.LivenessProbe != nil && ctr.LivenessProbe.SuccessThreshold != 1 {
			allErrs = append(allErrs, idxPath.Child("livenessProbe", "successThreshold").InvalidError(ctr.LivenessProbe.SuccessThreshold, "must be 1"))
		}
...
func validateRestartPolicy(restartPolicy *api.RestartPolicy, fldPath *field.Path) field.ErrorList {
	...
	default:
		...
		allErrors = append(allErrors, fldPath.NotSupportedError(*restartPolicy, validValues))
	}
...
func validateDNSPolicy(dnsPolicy *api.DNSPolicy, fldPath *field.Path) field.ErrorList {
	...
	default:
		...
		allErrors = append(allErrors, fldPath.NotSupportedError(dnsPolicy, validValues))
	}
	return allErrors
}
		...
		for i, container := range containers {
			...
			for i, port := range container.Ports {
				...
				if port.HostPort != port.ContainerPort {
					allErrors = append(allErrors, idxPath.Child("containerPort").InvalidError(port.ContainerPort, "must match `hostPort` when `hostNetwork` is true"))
				}
			}
		}
	}
...
func validateImagePullSecrets(imagePullSecrets []api.LocalObjectReference, fldPath *field.Path) field.ErrorList {
	...
	for i, currPullSecret := range imagePullSecrets {
		...
		if !reflect.DeepEqual(strippedRef, currPullSecret) {
			allErrors = append(allErrors, idxPath.InvalidError(currPullSecret, "only name may be set"))
		}
	}
	return allErrors
}
...
func ValidatePodSpec(spec *api.PodSpec, fldPath *field.Path) field.ErrorList {
	...
	if len(spec.ServiceAccountName) > 0 {
		if ok, msgs := ValidateServiceAccountName(spec.ServiceAccountName, false); !ok {
			for i := range msgs {
				allErrs = append(allErrs, fldPath.Child("serviceAccountName").InvalidError(spec.ServiceAccountName, msgs[i]))
			}
		}
	}

	if len(spec.NodeName) > 0 {
		if ok, msgs := ValidateNodeName(spec.NodeName, false); !ok {
			for i := range msgs {
				allErrs = append(allErrs, fldPath.Child("nodeName").InvalidError(spec.NodeName, msgs[i]))
			}
		}
	}
...
func ValidateNodeSelectorRequirement(rq api.NodeSelectorRequirement, fldPath *field.Path) field.ErrorList {
	...
	case api.NodeSelectorOpGt, api.NodeSelectorOpLt:
		if len(rq.Values) != 1 {
			allErrs = append(allErrs, fldPath.Child("values").RequiredError("must be specified single value when `operator` is 'Lt' or 'Gt'"))
		}
	default:
		allErrs = append(allErrs, fldPath.Child("operator").InvalidError(rq.Operator, "not a valid selector operator"))
	}
...
func ValidateNodeSelectorTerm(term api.NodeSelectorTerm, fldPath *field.Path) field.ErrorList {
	...
	if len(term.MatchExpressions) == 0 {
		return append(allErrs, fldPath.Child("matchExpressions").RequiredError("must have at least one node selector requirement"))
	}
...
func ValidateNodeSelector(nodeSelector *api.NodeSelector, fldPath *field.Path) field.ErrorList {
	...
	if len(nodeSelector.NodeSelectorTerms) == 0 {
		return append(allErrs, termFldPath.RequiredError("must have at least one node selector term"))
	}
...
func ValidatePreferredSchedulingTerms(terms []api.PreferredSchedulingTerm, fldPath *field.Path) field.ErrorList {
	...
	for i, term := range terms {
		if term.Weight <= 0 || term.Weight > 100 {
			allErrs = append(allErrs, fldPath.Index(i).Child("weight").InvalidError(term.Weight, "must be in the range 1-100"))
		}
...
func ValidateAffinityInPodAnnotations(annotations map[string]string, fldPath *field.Path) field.ErrorList {
	...
	if err != nil {
		allErrs = append(allErrs, fldPath.InvalidError(api.AffinityAnnotationKey, err.Error()))
		return allErrs
	}
	...
	if securityContext != nil {
		...
		if securityContext.FSGroup != nil {
			if ok, msgs := validation.IsGroupID(*securityContext.FSGroup); !ok {
				for i := range msgs {
					allErrs = append(allErrs, fldPath.Child("fsGroup").InvalidError(*(securityContext.FSGroup), msgs[i]))
				}
			}
		}
		if securityContext.RunAsUser != nil {
			if ok, msgs := validation.IsUserID(*securityContext.RunAsUser); !ok {
				for i := range msgs {
					allErrs = append(allErrs, fldPath.Child("runAsUser").InvalidError(*(securityContext.RunAsUser), msgs[i]))
				}
			}
		}
		for g, gid := range securityContext.SupplementalGroups {
			if ok, msgs := validation.IsGroupID(gid); !ok {
				for i := range msgs {
					allErrs = append(allErrs, fldPath.Child("supplementalGroups").Index(g).InvalidError(gid, msgs[i]))
				}
			}
		}
	}
...
func ValidatePodUpdate(newPod, oldPod *api.Pod) field.ErrorList {
	...
	if len(newPod.Spec.Containers) != len(oldPod.Spec.Containers) {
		...
		allErrs = append(allErrs, specPath.Child("containers").ForbiddenError("pod updates may not add or remove containers"))
		return allErrs
	}
	...
	for i, ctr := range newPod.Spec.Containers {
		if len(ctr.Image) == 0 {
			allErrs = append(allErrs, specPath.Child("containers").Index(i).Child("image").RequiredError(""))
		}
	}
	...
	if newPod.Spec.ActiveDeadlineSeconds != nil {
		...
		if oldPod.Spec.ActiveDeadlineSeconds != nil {
			...
			if oldActiveDeadlineSeconds < newActiveDeadlineSeconds {
				allErrs = append(allErrs, specPath.Child("activeDeadlineSeconds").InvalidError(newActiveDeadlineSeconds, "must be less than or equal to previous value"))
				return allErrs
			}
		}
	} else if oldPod.Spec.ActiveDeadlineSeconds != nil {
		allErrs = append(allErrs, specPath.Child("activeDeadlineSeconds").InvalidError(newPod.Spec.ActiveDeadlineSeconds, "must not update from a positive integer to nil value"))
	}
	...
	if !api.Semantic.DeepEqual(mungedPod.Spec, oldPod.Spec) {
		...
		allErrs = append(allErrs, specPath.ForbiddenError("pod updates may not change fields other than `containers[*].image` or `spec.activeDeadlineSeconds`"))
	}
...
func ValidatePodStatusUpdate(newPod, oldPod *api.Pod) field.ErrorList {
	...
	if newPod.Spec.NodeName != oldPod.Spec.NodeName {
		allErrs = append(allErrs, field.NewPath("status", "nodeName").ForbiddenError("may not be changed directly"))
	}
...
func ValidatePodBinding(binding *api.Binding) field.ErrorList {
	...
	if len(binding.Target.Kind) != 0 && binding.Target.Kind != "Node" {
		...
		allErrs = append(allErrs, field.NewPath("target", "kind").NotSupportedError(binding.Target.Kind, []string{"Node", "<empty>"}))
	}
	if len(binding.Target.Name) == 0 {
		// TODO: When validation becomes versioned, this gets more complicated.
		allErrs = append(allErrs, field.NewPath("target", "name").RequiredError(""))
	}
...
func ValidateService(service *api.Service) field.ErrorList {
	...
	if len(service.Spec.Ports) == 0 && service.Spec.ClusterIP != api.ClusterIPNone {
		allErrs = append(allErrs, specPath.Child("ports").RequiredError(""))
	}
	if service.Spec.Type == api.ServiceTypeLoadBalancer {
		for ix := range service.Spec.Ports {
			...
			if port.Port == 10250 {
				...
				allErrs = append(allErrs, portPath.InvalidError(port.Port, "may not expose port 10250 externally since it is used by kubelet"))
			}
		}
	}
	...
	if len(service.Spec.SessionAffinity) == 0 {
		allErrs = append(allErrs, specPath.Child("sessionAffinity").RequiredError(""))
	} else if !supportedSessionAffinityType.Has(string(service.Spec.SessionAffinity)) {
		allErrs = append(allErrs, specPath.Child("sessionAffinity").NotSupportedError(service.Spec.SessionAffinity, supportedSessionAffinityType.List()))
	}

	if api.IsServiceIPSet(service) {
		if ip := net.ParseIP(service.Spec.ClusterIP); ip == nil {
			allErrs = append(allErrs, specPath.Child("clusterIP").InvalidError(service.Spec.ClusterIP, "must be empty, 'None', or a valid IP address"))
		}
	}
	...
	if len(service.Spec.Type) == 0 {
		allErrs = append(allErrs, specPath.Child("type").RequiredError(""))
	} else if !supportedServiceType.Has(string(service.Spec.Type)) {
		allErrs = append(allErrs, specPath.Child("type").NotSupportedError(service.Spec.Type, supportedServiceType.List()))
	}
	...
	if service.Spec.Type == api.ServiceTypeLoadBalancer {
		...
		for i := range service.Spec.Ports {
			...
			if !supportedPortProtocols.Has(string(service.Spec.Ports[i].Protocol)) {
				allErrs = append(allErrs, portPath.Child("protocol").InvalidError(service.Spec.Ports[i].Protocol, "cannot create an external load balancer with non-TCP/UDP ports"))
			} else {
				includeProtocols.Insert(string(service.Spec.Ports[i].Protocol))
			}
		}
		if includeProtocols.Len() > 1 {
			allErrs = append(allErrs, portsPath.InvalidError(service.Spec.Ports, "cannot create an external load balancer with mix protocols"))
		}
	}
	...
	if service.Spec.Type == api.ServiceTypeClusterIP {
		...
		for i := range service.Spec.Ports {
			...
			if service.Spec.Ports[i].NodePort != 0 {
				allErrs = append(allErrs, portPath.Child("nodePort").InvalidError(service.Spec.Ports[i].NodePort, "may not be used when `type` is 'ClusterIP'"))
			}
		}
	}
	...
	for i := range service.Spec.Ports {
		...
		if found {
			allErrs = append(allErrs, portPath.Child("nodePort").DuplicateError(port.NodePort))
		}
		nodePorts[key] = true
	}
...
func validateServicePort(sp *api.ServicePort, requireName, isHeadlessService bool, allNames *sets.String, fldPath *field.Path) field.ErrorList {
	...
	if requireName && len(sp.Name) == 0 {
		...
		if ok, errs := validation.IsDNS1123Label(sp.Name); !ok {
			for i := range errs {
				allErrs = append(allErrs, fldPath.Child("name").InvalidError(sp.Name, errs[i]))
			}
		} else if allNames.Has(sp.Name) {
			allErrs = append(allErrs, fldPath.Child("name").DuplicateError(sp.Name))
		} else {
	...
	if ok, msgs := validation.IsPortNum(sp.Port); !ok {
		for i := range msgs {
			allErrs = append(allErrs, fldPath.Child("port").InvalidError(sp.Port, msgs[i]))
		}
	}

	if len(sp.Protocol) == 0 {
		allErrs = append(allErrs, fldPath.Child("protocol").RequiredError(""))
	} else if !supportedPortProtocols.Has(string(sp.Protocol)) {
		allErrs = append(allErrs, fldPath.Child("protocol").NotSupportedError(sp.Protocol, supportedPortProtocols.List()))
	}
	...
	if isHeadlessService {
		if sp.TargetPort.Type == intstr.String || (sp.TargetPort.Type == intstr.Int && sp.Port != sp.TargetPort.IntValue()) {
			allErrs = append(allErrs, fldPath.Child("port").InvalidError(sp.Port, "must be equal to targetPort when clusterIP = None"))
		}
	}
	...
	for i := range volumes {
		...
		if vol.GCEPersistentDisk != nil {
			if vol.GCEPersistentDisk.ReadOnly == false {
				allErrs = append(allErrs, idxPath.Child("gcePersistentDisk", "readOnly").InvalidError(false, "must be true for replicated pods > 1; GCE PD can only be mounted on multiple machines if it is read-only"))
			}
		}
...
func ValidateNode(node *api.Node) field.ErrorList {
	...
	if len(node.Spec.ExternalID) == 0 {
		allErrs = append(allErrs, field.NewPath("spec", "externalID").RequiredError(""))
	}
...
func ValidateNodeUpdate(node, oldNode *api.Node) field.ErrorList {
	...
	// 	allErrs = append(allErrs, "status".InvalidError(node.Status, "must be empty"))
	// }
	...
	for i, address := range node.Status.Addresses {
		if _, ok := addresses[address]; ok {
			allErrs = append(allErrs, field.NewPath("status", "addresses").Index(i).DuplicateError(address))
		}
	...
	if len(oldNode.Spec.PodCIDR) == 0 {
		...
		if oldNode.Spec.PodCIDR != node.Spec.PodCIDR {
			allErrs = append(allErrs, field.NewPath("spec", "podCIDR").ForbiddenError("node updates may not change podCIDR except from \"\" to valid"))
		}
	}
	...
	if !api.Semantic.DeepEqual(oldNode, node) {
		...
		allErrs = append(allErrs, field.NewPath("").ForbiddenError("node updates may only change labels or capacity"))
	}
...
func validateResourceName(value string, fldPath *field.Path) field.ErrorList {
	allErrs := field.ErrorList{}
	if ok, errs := validation.IsQualifiedName(value); !ok {
		for i := range errs {
			allErrs = append(allErrs, fldPath.InvalidError(value, errs[i]))
		}
		return allErrs
	}
	...
	if len(strings.Split(value, "/")) == 1 {
		if !api.IsStandardResourceName(value) {
			return append(allErrs, fldPath.InvalidError(value, "must be a standard resource type or fully qualified"))
		}
	}
...
func ValidateLimitRange(limitRange *api.LimitRange) field.ErrorList {
	...
	for i := range limitRange.Spec.Limits {
		...
		if found {
			allErrs = append(allErrs, idxPath.Child("type").DuplicateError(limit.Type))
		}
		...
		if limit.Type == api.LimitTypePod {
			if len(limit.Default) > 0 {
				allErrs = append(allErrs, idxPath.Child("default").ForbiddenError("may not be specified when `type` is 'Pod'"))
			}
			if len(limit.DefaultRequest) > 0 {
				allErrs = append(allErrs, idxPath.Child("defaultRequest").ForbiddenError("may not be specified when `type` is 'Pod'"))
			}
		} else {
		...
		for k := range keys {
			...
			if minQuantityFound && maxQuantityFound && minQuantity.Cmp(maxQuantity) > 0 {
				allErrs = append(allErrs, idxPath.Child("min").Key(string(k)).InvalidError(minQuantity, fmt.Sprintf("min value %s is greater than max value %s", minQuantity.String(), maxQuantity.String())))
			}

			if defaultRequestQuantityFound && minQuantityFound && minQuantity.Cmp(defaultRequestQuantity) > 0 {
				allErrs = append(allErrs, idxPath.Child("defaultRequest").Key(string(k)).InvalidError(defaultRequestQuantity, fmt.Sprintf("min value %s is greater than default request value %s", minQuantity.String(), defaultRequestQuantity.String())))
			}

			if defaultRequestQuantityFound && maxQuantityFound && defaultRequestQuantity.Cmp(maxQuantity) > 0 {
				allErrs = append(allErrs, idxPath.Child("defaultRequest").Key(string(k)).InvalidError(defaultRequestQuantity, fmt.Sprintf("default request value %s is greater than max value %s", defaultRequestQuantity.String(), maxQuantity.String())))
			}

			if defaultRequestQuantityFound && defaultQuantityFound && defaultRequestQuantity.Cmp(defaultQuantity) > 0 {
				allErrs = append(allErrs, idxPath.Child("defaultRequest").Key(string(k)).InvalidError(defaultRequestQuantity, fmt.Sprintf("default request value %s is greater than default limit value %s", defaultRequestQuantity.String(), defaultQuantity.String())))
			}

			if defaultQuantityFound && minQuantityFound && minQuantity.Cmp(defaultQuantity) > 0 {
				allErrs = append(allErrs, idxPath.Child("default").Key(string(k)).InvalidError(minQuantity, fmt.Sprintf("min value %s is greater than default value %s", minQuantity.String(), defaultQuantity.String())))
			}

			if defaultQuantityFound && maxQuantityFound && defaultQuantity.Cmp(maxQuantity) > 0 {
				allErrs = append(allErrs, idxPath.Child("default").Key(string(k)).InvalidError(maxQuantity, fmt.Sprintf("default value %s is greater than max value %s", defaultQuantity.String(), maxQuantity.String())))
			}
			if maxRatioFound && maxRatio.Cmp(*resource.NewQuantity(1, resource.DecimalSI)) < 0 {
				allErrs = append(allErrs, idxPath.Child("maxLimitRequestRatio").Key(string(k)).InvalidError(maxRatio, fmt.Sprintf("ratio %s is less than 1", maxRatio.String())))
			}
			if maxRatioFound && minQuantityFound && maxQuantityFound {
				...
				if maxRatioValue > maxRatioLimit {
					allErrs = append(allErrs, idxPath.Child("maxLimitRequestRatio").Key(string(k)).InvalidError(maxRatio, fmt.Sprintf("ratio %s is greater than max/min = %f", maxRatio.String(), maxRatioLimit)))
				}
			}
		}
	}
...
func ValidateSecret(secret *api.Secret) field.ErrorList {
	...
	for key, value := range secret.Data {
		if !IsSecretKey(key) {
			allErrs = append(allErrs, dataPath.Key(key).InvalidError(key, fmt.Sprintf("must have at most %d characters and match regex %s", validation.DNS1123SubdomainMaxLength, SecretKeyFmt)))
		}
		totalSize += len(value)
	}
	if totalSize > api.MaxSecretSize {
		allErrs = append(allErrs, dataPath.TooLongError("", api.MaxSecretSize))
	}
	...
	case api.SecretTypeServiceAccountToken:
		...
		if value := secret.Annotations[api.ServiceAccountNameKey]; len(value) == 0 {
			allErrs = append(allErrs, field.NewPath("metadata", "annotations").Key(api.ServiceAccountNameKey).RequiredError(""))
		}
	...
	case api.SecretTypeDockercfg:
		...
		if !exists {
			allErrs = append(allErrs, dataPath.Key(api.DockerConfigKey).RequiredError(""))
			break
		}

		// make sure that the content is well-formed json.
		if err := json.Unmarshal(dockercfgBytes, &map[string]interface{}{}); err != nil {
			allErrs = append(allErrs, dataPath.Key(api.DockerConfigKey).InvalidError("<secret contents redacted>", err.Error()))
		}
	case api.SecretTypeDockerConfigJson:
		dockerConfigJsonBytes, exists := secret.Data[api.DockerConfigJsonKey]
		if !exists {
			allErrs = append(allErrs, dataPath.Key(api.DockerConfigJsonKey).RequiredError(""))
			break
		}

		// make sure that the content is well-formed json.
		if err := json.Unmarshal(dockerConfigJsonBytes, &map[string]interface{}{}); err != nil {
			allErrs = append(allErrs, dataPath.Key(api.DockerConfigJsonKey).InvalidError("<secret contents redacted>", err.Error()))
		}
	case api.SecretTypeBasicAuth:
		...
		if !usernameFieldExists && !passwordFieldExists {
			allErrs = append(allErrs, field.NewPath("data[%s]").Key(api.BasicAuthUsernameKey).RequiredError(""))
			allErrs = append(allErrs, field.NewPath("data[%s]").Key(api.BasicAuthPasswordKey).RequiredError(""))
			break
		}
	case api.SecretTypeSSHAuth:
		if len(secret.Data[api.SSHAuthPrivateKey]) == 0 {
			allErrs = append(allErrs, field.NewPath("data[%s]").Key(api.SSHAuthPrivateKey).RequiredError(""))
			break
		}

	case api.SecretTypeTLS:
		if _, exists := secret.Data[api.TLSCertKey]; !exists {
			allErrs = append(allErrs, dataPath.Key(api.TLSCertKey).RequiredError(""))
		}
		if _, exists := secret.Data[api.TLSPrivateKeyKey]; !exists {
			allErrs = append(allErrs, dataPath.Key(api.TLSPrivateKeyKey).RequiredError(""))
		}
...
func ValidateConfigMap(cfg *api.ConfigMap) field.ErrorList {
	...
	for key, value := range cfg.Data {
		if !IsSecretKey(key) {
			allErrs = append(allErrs, field.NewPath("data").Key(key).InvalidError(key, fmt.Sprintf("must have at most %d characters and match regex %s", validation.DNS1123SubdomainMaxLength, SecretKeyFmt)))
		}
		totalSize += len(value)
	}
	if totalSize > api.MaxSecretSize {
		allErrs = append(allErrs, field.NewPath("data").TooLongError("", api.MaxSecretSize))
	}
...
func validateBasicResource(quantity resource.Quantity, fldPath *field.Path) field.ErrorList {
	if quantity.Value() < 0 {
		return field.ErrorList{fldPath.InvalidError(quantity.Value(), "must be a valid resource quantity")}
	}
	return field.ErrorList{}
}
	...
	for resourceName, quantity := range requirements.Limits {
		...
		if exists {
			...
			if limitValue < requestValue {
				allErrs = append(allErrs, fldPath.InvalidError(quantity.String(), "must be greater than or equal to request"))
			}
		}
	}
...
func validateResourceQuantityValue(resource string, value resource.Quantity, fldPath *field.Path) field.ErrorList {
	...
	if api.IsIntegerResourceName(resource) {
		if value.MilliValue()%int64(1000) != int64(0) {
			allErrs = append(allErrs, fldPath.InvalidError(value, "must be a whole number"))
		}
	}
	return allErrs
}
...
func validateFinalizerName(stringValue string, fldPath *field.Path) field.ErrorList {
	allErrs := field.ErrorList{}
	if ok, errs := validation.IsQualifiedName(stringValue); !ok {
		for i := range errs {
			allErrs = append(allErrs, fldPath.InvalidError(stringValue, errs[i]))
		}
		return allErrs
	}
	...
	if len(strings.Split(stringValue, "/")) == 1 {
		if !api.IsStandardFinalizerName(stringValue) {
			return append(allErrs, fldPath.InvalidError(stringValue, fmt.Sprintf("name is neither a standard finalizer name nor is it fully qualified")))
		}
	}
...
func ValidateNamespaceStatusUpdate(newNamespace, oldNamespace *api.Namespace) field.ErrorList {
	...
	if newNamespace.DeletionTimestamp.IsZero() {
		if newNamespace.Status.Phase != api.NamespaceActive {
			allErrs = append(allErrs, field.NewPath("status", "phase").InvalidError(newNamespace.Status.Phase, "may only be 'Active' if `deletionTimestamp` is empty"))
		}
	} else {
		if newNamespace.Status.Phase != api.NamespaceTerminating {
			allErrs = append(allErrs, field.NewPath("status", "phase").InvalidError(newNamespace.Status.Phase, "may only be 'Terminating' if `deletionTimestamp` is not empty"))
		}
	}
	return allErrs
}
...
func validateEndpointSubsets(subsets []api.EndpointSubset, fldPath *field.Path) field.ErrorList {
	...
	for i := range subsets {
		...
		if len(ss.Addresses) == 0 && len(ss.NotReadyAddresses) == 0 {
			...
			allErrs = append(allErrs, idxPath.RequiredError("must specify `addresses` or `notReadyAddresses`"))
		}
		if len(ss.Ports) == 0 {
			allErrs = append(allErrs, idxPath.Child("ports").RequiredError(""))
		}
...
func validateNonSpecialIP(ipAddress string, fldPath *field.Path) field.ErrorList {
	...
	if ok, msgs := validation.IsNonSpecialIPv4(ipAddress); !ok {
		for i := range msgs {
			allErrs = append(allErrs, fldPath.InvalidError(ipAddress, msgs[i]))
		}
	}
	return allErrs
}
...
func validateEndpointPort(port *api.EndpointPort, requireName bool, fldPath *field.Path) field.ErrorList {
	...
	if requireName && len(port.Name) == 0 {
		...
		if ok, errs := validation.IsDNS1123Label(port.Name); !ok {
			for i := range errs {
				allErrs = append(allErrs, fldPath.Child("name").InvalidError(port.Name, errs[i]))
			}
		}
	}
	if ok, msgs := validation.IsPortNum(port.Port); !ok {
		for i := range msgs {
			allErrs = append(allErrs, fldPath.Child("port").InvalidError(port.Port, msgs[i]))
		}
	}
	if len(port.Protocol) == 0 {
		allErrs = append(allErrs, fldPath.Child("protocol").RequiredError(""))
	} else if !supportedPortProtocols.Has(string(port.Protocol)) {
		allErrs = append(allErrs, fldPath.Child("protocol").NotSupportedError(port.Protocol, supportedPortProtocols.List()))
	}
	return allErrs
}
...
func ValidateSecurityContext(sc *api.SecurityContext, fldPath *field.Path) field.ErrorList {
	...
	if sc.Privileged != nil {
		if *sc.Privileged && !capabilities.Get().AllowPrivileged {
			allErrs = append(allErrs, fldPath.Child("privileged").ForbiddenError("disallowed by policy"))
		}
	}
...
func ValidatePodLogOptions(opts *api.PodLogOptions) field.ErrorList {
	...
	case opts.SinceSeconds != nil && opts.SinceTime != nil:
		allErrs = append(allErrs, field.NewPath("").ForbiddenError("at most one of `sinceTime` or `sinceSeconds` may be specified"))
	...
	for i, ingress := range status.Ingress {
		...
		if len(ingress.IP) > 0 {
			if isIP := (net.ParseIP(ingress.IP) != nil); !isIP {
				allErrs = append(allErrs, idxPath.Child("ip").InvalidError(ingress.IP, "must be a valid IP address"))
			}
		}
		if len(ingress.Hostname) > 0 {
			if valid, msgs := NameIsDNSSubdomain(ingress.Hostname, false); !valid {
				for i := range msgs {
					allErrs = append(allErrs, idxPath.Child("hostname").InvalidError(ingress.Hostname, msgs[i]))
				}
			}
			if isIP := (net.ParseIP(ingress.Hostname) != nil); isIP {
				allErrs = append(allErrs, idxPath.Child("hostname").InvalidError(ingress.Hostname, "must be a DNS name, not an IP address"))
			}
		}
	}
	return allErrs
