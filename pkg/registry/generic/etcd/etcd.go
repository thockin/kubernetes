...
func NamespaceKeyFunc(ctx api.Context, prefix string, name string) (string, error) {
	...
	if ok, msgs := validation.IsValidPathSegmentName(name); !ok {
		return "", kubeerr.NewBadRequest(fmt.Sprintf("Name parameter invalid: %v.", msgs))
	}
	key = key + "/" + name
	return key, nil
}
	...
	err = e.Storage.GuaranteedUpdate(ctx, key, out, true, func(existing runtime.Object, res storage.ResponseMeta) (runtime.Object, *uint64, error) {
		...
		if doUnconditionalUpdate {
			...
			if newVersion == 0 {
				...
				fieldErrList := field.ErrorList{field.NewPath("metadata").Child("resourceVersion").InvalidError(newVersion, "must be specified for an update")}
