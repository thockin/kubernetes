...
func (rs *REST) Create(ctx api.Context, obj runtime.Object) (runtime.Object, error) {
	...
	if api.IsServiceIPRequested(service) {
		...
		if err := rs.serviceIPs.Allocate(net.ParseIP(service.Spec.ClusterIP)); err != nil {
			...
			el := field.ErrorList{field.NewPath("spec", "clusterIP").InvalidError(service.Spec.ClusterIP, err.Error())}
	...
	for i := range service.Spec.Ports {
		...
		if servicePort.NodePort != 0 {
			...
			if err != nil {
				// TODO: when validation becomes versioned, this gets more complicated.
				el := field.ErrorList{field.NewPath("spec", "ports").Index(i).Child("nodePort").InvalidError(servicePort.NodePort, err.Error())}
				return nil, errors.NewInvalid(api.Kind("Service"), service.Name, el)
			}
		...
		for i := range service.Spec.Ports {
			...
			if nodePort != 0 {
				if !contains(oldNodePorts, nodePort) {
					...
					if err != nil {
						el := field.ErrorList{field.NewPath("spec", "ports").Index(i).Child("nodePort").InvalidError(nodePort, err.Error())}
